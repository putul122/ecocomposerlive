...
function force (graphData) {
    ...
    var s = new XMLSerializer().serializeToString(document.getElementById('diagramLayout'))
    console.log(document.getElementById('diagramLayout'))
    console.log(s)
    ...
    console.log('Data Model ---------->>', props)
        if (Object.keys(props.startNode).length > 0 && props.startNode.constructor === Object) {
            if (props.relationships && props.relationships) {
                ...
                // Setting first node
                node.id = 0
                node.name = props.startNode.name
                node.Title = props.startNode.title
                node.width = 140
                node.height = 70
                node.x = 400
                node.y = 450
